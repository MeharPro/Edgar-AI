...
async function upsertUser(supabase, key, payload) {
  ...
  if (key.includes('@')) {
    ...
    const { data, error } = await supabase.from('users').select('id, plan').eq('email', key).single();
    existingUser = data;
    selectError = error;
  } else {
    // Key is a UUID, search by ID
    const { data, error } = await supabase.from('users').select('id, plan').eq('id', key).single();
    existingUser = data;
    selectError = error;
  }
  if (selectError) {
